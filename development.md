...
- Inicie o stack local com Docker Compose:
...
**Nota**: na primeira execução o backend aguarda o banco e prepara o ambiente.
...
O Mailcatcher é um servidor SMTP simples que captura e-mails enviados pelo backend durante o desenvolvimento.

Isso é útil para:

- Testar recuperação de senha por e-mail
- Verificar conteúdo e formatação
- Depurar fluxos de e-mail sem envio real

No ambiente local via Docker Compose ele usa SMTP na porta `1025`, e a interface web fica em <http://localhost:1080>.

## Desenvolvimento Local

O Docker Compose expõe as portas do backend e frontend nas mesmas portas usadas no desenvolvimento local. Assim, você pode parar um serviço no Compose e rodar o servidor localmente sem alterar URLs.

Exemplos:

Parar o frontend do Compose e rodar localmente:
...
Parar o backend do Compose e rodar localmente:
...
## Docker Compose com `localhost.tiangolo.com`

Por padrão, o stack local usa `localhost` com portas distintas. Para simular subdomínios locais (como em produção), altere o `.env`:

```dotenv
DOMAIN=localhost.tiangolo.com
```

Isso habilita, por exemplo:
...
## Arquivos Docker Compose e `.env`

- O arquivo principal é `compose.yml` (equivalente ao `docker-compose.yml`).
- O `compose.override.yml` aplica ajustes de desenvolvimento, como volumes e hot reload.

Sempre que mudar o `.env`, reinicie o stack:
...
Usamos o [prek](https://prek.j178.dev/) para lint/format antes do commit.

Instalar hooks (no `backend`):
...
### Desenvolvimento com `localhost.tiangolo.com`
...
- Mailcatcher: <http://localhost.tiangolo.com:1080># FastAPI Project - Development
