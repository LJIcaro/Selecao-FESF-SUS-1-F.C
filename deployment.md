...
Você pode implantar o dashboard usando Docker Compose em um servidor remoto.

Este projeto espera um proxy Traefik para lidar com tráfego externo e certificados HTTPS.

Há também configurações para CI/CD com GitHub Actions.

## Preparação

- Tenha um servidor remoto pronto.
- Configure DNS do domínio para o IP do servidor.
- Configure um wildcard de subdomínio, por exemplo `*.dashboard.exemplo.com` para acessar serviços como:
  - `dashboard.dashboard.exemplo.com`
  - `api.dashboard.exemplo.com`
  - `traefik.dashboard.exemplo.com`
  - `adminer.dashboard.exemplo.com`
- Instale [Docker Engine](https://docs.docker.com/engine/install/) no servidor remoto.

## Traefik Público

### Docker Compose do Traefik

Crie um diretório remoto:

```bash
mkdir -p /root/code/traefik-public/
```

Copie o `compose.traefik.yml`:

```bash
rsync -a compose.traefik.yml root@seu-servidor.exemplo.com:/root/code/traefik-public/
```

### Rede pública do Traefik

Crie a rede pública usada pelos stacks:

```bash
docker network create traefik-public
```

### Variáveis do Traefik

No servidor remoto, defina:

```bash
export USERNAME=admin
export PASSWORD=changethis
export HASHED_PASSWORD=$(openssl passwd -apr1 $PASSWORD)
export DOMAIN=dashboard.exemplo.com
...
## Implantar o Dashboard

### Copiar o código

```bash
rsync -av --filter=":- .gitignore" ./ root@seu-servidor.exemplo.com:/root/code/app/
```

### Variáveis de ambiente

Gere chaves secretas:
...
export FIRST_SUPERUSER_PASSWORD="sua_senha_admin"
...
- `PROJECT_NAME`: Nome do dashboard.
- `STACK_NAME`: Nome da stack do Compose.
- `FIRST_SUPERUSER`: E-mail do superusuário.
- `SMTP_HOST`, `SMTP_USER`, `SMTP_PASSWORD`, `EMAILS_FROM_EMAIL`: SMTP para e-mails.
- `POSTGRES_SERVER`, `POSTGRES_PORT`, `POSTGRES_USER`, `POSTGRES_DB`: Banco de dados.
- `SENTRY_DSN`: DSN do Sentry.

### Deploy com Docker Compose

Em produção, use apenas `compose.yml` (equivalente ao `docker-compose.yml`):
...
Você pode configurar um runner self-hosted no servidor:
...
Siga o guia oficial do GitHub para instalar o runner no repositório.

Depois, instale como serviço para garantir execução automática.

## Observações da Stack

- A autenticação é baseada em **OAuth2**.
- O cache com **Redis** deve estar disponível na infraestrutura quando habilitado.
- A conteinerização usa `Dockerfile` e `compose.yml` (equivalente a `docker-compose.yml`).# FastAPI Project - Deployment
